...
namespace WebApplication1.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> ResetPassword(ResetPassword vm)
        {
            ...
            {
                _notification.Error("l'utente non esiste");
                return View(vm);
            }
            var token = await _userManager.GeneratePasswordResetTokenAsync(existingUser);
            var result = await _userManager.ResetPasswordAsync(existingUser, "",vm.NewPassword);
            if (result.Succeeded)
            {
                _notification.Success("la password è stata modificata correttamente!!");
        ...
        public async Task<IActionResult> Register(RegisterVm vm)
        {
            ...
            {
                _notification.Error("l'email esiste già");
                    return View(vm);
            }
            var checkUserByUsername = await _userManager.FindByNameAsync(vm.UserName);
            if (checkUserByUsername != null)
            {
                _notification.Error("l'utente esiste già");
                    return View(vm);
            }
            ...
            if(result.Succeeded)
            {
                ...
                _notification.Success("Utente creato con successo!!");
